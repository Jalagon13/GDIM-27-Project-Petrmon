    ...
    public class StatMoveSpeed : StatMove
    {
        public sealed override string Execute(PetrmonObject fromPetrmon, PetrmonObject toPetrmon)
        {
            ...
            if (_useOnOwnPetrmon)
            {
                ...
                return $"{fromPetrmon.Name.ToUpper()}'s Speed rose!";
            }
            else
            {
                toPetrmon.BattleStats.AlterSpeed(_multiplier);
                return _multiplier > 1 ? $"{toPetrmon.Name.ToUpper()}'s Speed rose!" :
                    _multiplier < 1 ? $"{toPetrmon.Name.ToUpper()}'s Speed fell!" : "ERROR MULTIPLER CAN'T BE 0!";
            }
        }
    }
}
